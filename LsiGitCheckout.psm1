...
function Invoke-GitCheckout {
    ...
    $basePath = $Repository.'Base Path'
    $tag = $Repository.Tag
    $skipLfs = $Repository.'Skip LFS' ?? $false
    $wasNewClone = $false
    ...
    if (Test-Path $absoluteBasePath) {
        ...
        if (Test-Path $gitDir) {
            ...
            if ($existingUrl -eq $repoUrl) {
                ...
                $skipLfsValue = [bool]($skipLfs ?? $false)
            ...
            try {
                ...
                $repoToAdd = Invoke-WithErrorContext -Context "Processing repository: $($repo.'Repository URL')" -ScriptBlock {
                    if (Invoke-GitCheckout -Repository $repo -DependencyFilePath $DependencyFilePath -CallingRepositoryRootPath $CallingRepositoryRootPath) {
                        ...
                        if ($script:RecursiveMode -and $Depth -lt $script:MaxDepth -and $checkoutSucceeded) {
...
function Process-RecursiveDependencies {
    ...
    if ($CurrentDepth -ge $script:MaxDepth) {
        Write-Log "Maximum recursion depth ($script:MaxDepth) reached" -Level Warning
        return
    }
...
"@
    ...
    if ($script:RecursiveMode) {
        ...
        $summary += "`nMax Depth: $script:MaxDepth"
    ...
    if ($script:EnableDebug -and $script:RepositoryDictionary.Count -gt 0) {
        $totalTags = 0
        $reposWithTags = 0
        $totalSemVerTags = 0
